...
import { MessageEnvelope, SendMessagePayload } from '../models/message';

/** Returns true if the given userId has at least one active socket across all server instances. */
const isRecipientConnected = async (
  io: SocketIOServer,
  userId: string,
): Promise<boolean> => {
...
export const registerSocketHandlers = (io: SocketIOServer): void => {
  io.on('connection', (socket: Socket) => {
    ...
    offlineQueueService.drain(userId, socket).catch((err: Error) =>
    ...
    socket.on('send_message', async (msg: SendMessagePayload) => {
      ...
      if (!to || typeof to !== 'string') {
        console.warn(
          `[Socket] send_message ignored — invalid 'to' | socket=${socket.id}`,
        );
        return;
      }
      ...
      console.log(
        `[Socket] send_message | from=${userId} | to=${to} | ` +
        `payloadType=${typeof payload} | ts=${sentAt}`,
      );

      const envelope: MessageEnvelope = { from: userId, payload, sentAt };
    ...
    socket.on('disconnect', (reason: string) => {
      console.log(
        `[Socket] Disconnected | socket=${socket.id} | room=${userId} | reason=${reason}`,
      );
    });
  });
};
