...
const queueKey = (userId: string): string => `offline_queue:${userId}`;

/** 24 hours — messages expire from Redis after this TTL. */
const OFFLINE_QUEUE_TTL_SECONDS = 86_400;
...
export const offlineQueueService = {
   ...
   */
  async enqueue(recipientId: string, envelope: MessageEnvelope): Promise<void> {
    const key = queueKey(recipientId);
    await storeClient.rpush(key, JSON.stringify(envelope));
    await storeClient.expire(key, OFFLINE_QUEUE_TTL_SECONDS);
    console.log(
      `[OfflineQueue] Queued | to=${recipientId} | ttl=${OFFLINE_QUEUE_TTL_SECONDS}s`,
    );
  },
   ...
   */
  async drain(userId: string, socket: Socket): Promise<void> {
    const key = queueKey(userId);
    const raw = await storeClient.lrange(key, 0, -1);
    ...
    for (const item of raw) {
      try {
        const envelope = JSON.parse(item) as MessageEnvelope;
